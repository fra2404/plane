...
class OIDCOAuthProvider(OauthAdapter):
    ...
    def __init__(self, request, code=None, state=None, callback=None, nonce=None, code_verifier=None):
        ...
        # Build the callback from the configured public URL (WEB_URL/APP_BASE_URL)
        # instead of the request scheme: when Plane runs behind a proxy that
        # terminates TLS (Cloudflare -> Caddy), Django sees http even though the
        # public origin is https, which would produce an invalid redirect_uri.
